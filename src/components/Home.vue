  ...
  <div id="students">
    <table>
      ...
      <tr v-for="(student, index) in students" :key="student.index">
        ...
        <td
          v-bind:class="{
            failed: ComputGrad(student) < 60,
            passed: ComputGrad(student) >= 60,
          }"
        >
          {{ ComputGrad(student) >= 60 ? "Passed" : "Failed" }}
        </td>
        <td>
          <button @click="deletlesson(index)"><em>&times;</em></button>
        </td>
      </tr>
    </table>
